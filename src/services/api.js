...
async function safeFetch(input, init, fallback) {
    try {
...
export async function fetchMeetings() {
    return safeFetch(`${API_BASE_URL}/meetings`, undefined, () => mockMeetings);
}
export async function fetchSummary(meetingId) {
    try {
...
export async function uploadTranscription(meetingId, file) {
    try {
        ...
        return (data.highlights ?? []).map((item, index) => ({
            id: `trans-${index}`,
...
export async function fetchTranscriptSegments(meetingId) {
    const fallback = () => mockSegments.map((segment) => ({
        start: segment.start,
        end: segment.end,
        text: segment.text,
        speaker: segment.speaker,
    }));
    const rawSegments = await safeFetch(`${API_BASE_URL}/meetings/${meetingId}/transcript`, undefined, fallback);
